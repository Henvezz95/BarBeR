...
extern "C" DLL_PUBLIC void ravel(int* result, std::vector<std::vector<cv::Point>> input);


extern "C" DLL_PUBLIC int adder(int a, int b);
 ...
 * @return
 */
extern "C" DLL_PUBLIC std::vector<std::vector<cv::Point>> getLineSegmentsContours(std::vector<cv::line_descriptor::KeyLine> &keylines,
																														cv::Mat& image_lines,
																														int minLineLength);
 ...
 */
extern "C" DLL_PUBLIC void findContainingSegments(std::vector<std::vector<std::shared_ptr<cv::line_descriptor::KeyLine>>> &keylinesInContours,
 ...
 */
extern "C" DLL_PUBLIC void calculateSupportScores(std::vector<std::vector<std::shared_ptr<cv::line_descriptor::KeyLine>>> &keylinesInContours,
 ...
 * @param support_candidates_threshold Threshold for the support value under which contours are ignored.
 */
extern "C" DLL_PUBLIC void selectSCand(std::vector<std::vector<int>> &support_scores,
 ...
 */
extern "C" DLL_PUBLIC void createVectorsOfIntensities(std::vector<int> &support_candidates,
																std::vector<int> &support_candidates_pos,
																std::vector<std::vector<std::shared_ptr<cv::line_descriptor::KeyLine>>> &keylinesInContours,
 ...
 * @param deletedContours Binary vector which indicates which contours were deleted (deleted = true, not deleted = false).
 */
extern "C" DLL_PUBLIC void computePhis(int delta,
								 std::vector<std::vector<std::vector<uchar>>> &intensities,
								 int intensities_size,
 ...
 * @param minLengthToLineLengthRatio Threshold to ignore contours with a too low length / line-length ratio.
 */
extern "C" DLL_PUBLIC void calculateBoundingBoxes(int keylinesInContours_size,
														std::vector<int> &start_barcode_pos,
														std::vector<int> &end_barcode_pos,
														std::vector<cv::line_descriptor::KeyLine> &keylines,
 ...
 * @param inSegmentYDistance Distance in y-direction for segements within a contour. Used as filter parameter.
 */
extern "C" DLL_PUBLIC void filterContours(int keylinesInContours_size,
										std::vector<bool> &deletedContours,
										std::vector<int> &start_barcode_pos,
										std::vector<int> &end_barcode_pos,
										std::vector<cv::line_descriptor::KeyLine> &keylines,
 ...
 */
extern "C" DLL_PUBLIC cv::Point contourCenter(const std::vector<cv::Point>& contour);
 ...
 */
extern "C" DLL_PUBLIC std::vector<cv::Point> scaleContour(double scalingFactor,
 ...
 */
extern "C" DLL_PUBLIC cv::Rect clamRoiToImage(cv::Rect roi, const cv::Mat& image);
