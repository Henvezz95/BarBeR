...
def reading_rate(decoded, not_decoded):
    return decoded / (not_decoded + decoded) if decoded+not_decoded > 0 else 0
...
if __name__ == "__main__":
    ...
    NUM_REPEATS = 3
    single_code = test_config["single_ROI"] if "single_ROI" in test_config else False
    if test_config["class"] == '1D':
        class_id = 1
    elif test_config["class"] == '2D':
        class_id = 2
    else:
        class_id = 3
    ...
    detectors = {}
    readers = {'None':ZbarReader(pre_localizer=None, optimize=False)}
    results = {'None':[]}
    ...
    for file_name, metadata in tqdm(datasets_info['images'].items()):
        ...
        if class_id != 3 and any(x != class_id for x in true_classes):
            continue
