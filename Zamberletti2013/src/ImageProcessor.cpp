    ...
    void ImageProcessor::locate(int* result, double* angle, int* num_results, unsigned char* img_color, int h, int w){
        ...
        *angle = max_angle_hist(get_histogram(img_neural, HIST_ROW));
        ...
        foreach(cv::Rect r, rects){
            result[0+(*num_results)*8] = r.x;
            result[1+(*num_results)*8] = r.y;
            result[2+(*num_results)*8] = r.x + r.width;
            result[3+(*num_results)*8] = r.y;
            result[4+(*num_results)*8] = r.x + r.width;
            result[5+(*num_results)*8] = r.y + r.height;
            result[6+(*num_results)*8] = r.x;
            result[7+(*num_results)*8] = r.y + r.height;
            (*num_results)++;
        }
        rects.clear();
    }
