...
if __name__ == "__main__":
    ...
    def train_one_epoch(cuda=True, crops=False):
        ...
        for i, data in tqdm(enumerate(dataloader_train)):
            inputs, labels = data
            if np.random.uniform(0,1)>0.5 and crops:
                inputs_numpy = inputs.cpu().detach().numpy()
                labels_numpy = labels.cpu().detach().numpy()
                num_H = np.random.choice([10,5,4,2])
                num_W = np.random.choice([10,5,4,2])
                input_crops = []
                label_crops_0 = []
                label_crops_1 = []
                for x in inputs_numpy:
                    input_crops.extend(make_crops_from_image(x[0], num_H, num_W, 0, 0))
                for x in labels_numpy:
                    label_crops_0.extend(make_crops_from_image(x[0], num_H, num_W, 0, 0))
                    label_crops_1.extend(make_crops_from_image(x[1], num_H, num_W, 0, 0))
                inputs = torch.from_numpy(np.array(input_crops, dtype=np.float32)[:,np.newaxis])
                label_crops = np.array([label_crops_0,label_crops_1], dtype=np.float32)
                label_crops = np.transpose(label_crops, (1,0,2,3))
                labels = torch.from_numpy(label_crops)
